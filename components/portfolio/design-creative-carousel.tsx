...
const categories: Category[] = [
  ...
  { label: 'Social Media Design', description: 'Educational & campaign content', icon: Smartphone, slides: [] },
    ...
    <section className="bg-[#F8F7FF] py-20" aria-labelledby="design-creative-heading">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {slides.length ? <>
          ...
          <div className="mt-8"><h3 className="mb-4 text-lg font-bold text-[#0F0A2E]">More Marketplace Banners</h3><div className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">{slides.map((slide, index) => <button key={slide.src} type="button" onClick={() => goTo(index)} className={`overflow-hidden rounded-xl border-2 bg-white text-left ${activeIndex === index ? 'border-[#2D1BB8]' : 'border-transparent'}`}><div className="relative aspect-[16/9]"><Image src={slide.src} alt={slide.title} fill sizes="160px" className="object-cover" /></div><span className="block truncate px-2 py-2 text-xs text-[#4B4680]">{slide.title}</span></button>)}</div></div>
